...
document.getElementById("fileInput").onchange=async e=>{
    for(let file of e.target.files){
        let f=new FormData();
        f.append("image",file);
        let res=await fetch("/remove-bg",{method:"POST",body:f});
...
function applySnapping(c){
    ...
    let ccx=canvas.width/2;
    let ccy=canvas.height/2;

    if(Math.abs(cx-ccx)<SNAP_THRESHOLD){
        c.x=ccx-w/2;
        showVGuide=true;
    }

    if(Math.abs(cy-ccy)<SNAP_THRESHOLD){
        c.y=ccy-h/2;
        showHGuide=true;
    }
}
...
// touch + pinch
let dragging=false,lastX,lastY,lastDist=null;
...
function smartCenter(){
    ...
    t.drawImage(c.img,0,0);
    let data=t.getImageData(0,0,tmp.width,tmp.height).data;

    let minX=9999,minY=9999,maxX=0,maxY=0;

    for(let y=0;y<tmp.height;y++){
        for(let x=0;x<tmp.width;x++){
            let i=(y*tmp.width+x)*4;
            if(data[i+3]>10){
                if(x<minX)minX=x;
                if(y<minY)minY=y;
                if(x>maxX)maxX=x;
